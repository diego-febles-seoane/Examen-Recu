...
public class FileArmasCsv {
    ...
    public void deleteArma(Arma arma) {
        List<Arma> armas = obtenerArmas();
        boolean eliminado = armas.removeIf(a -> a.getId().equals(arma.getId()));
        
        if (eliminado) {
            updateFile(armas, file);
            System.out.println("Arma eliminada exitosamente.");
        } else {
            System.err.println("El arma con ID " + arma.getId() + " no existe.");
        }
    }
    ...
    public void updateArma(Arma arma) {
        ...
        for (int i = 0; i < armas.size(); i++) {
            if (armas.get(i).getId().equals(arma.getId())) {
                armas.set(i, arma);
                actualizado = true;
                break;
            }
